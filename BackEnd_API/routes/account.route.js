...
router.post("/", async (req, res) => {
  ...
  if (isValid) {
    ...
    const results = await userAccount.add(entity);

    let account_num1 = common.genagrateAccountNumber(dob, count);
    console.log("insertId : " + JSON.stringify(results));
    entity = {
    ...
    await bankingInfoModel.add(entity);
    restItem.account = [
      { accountNum: account_num1, type: 1 },
      { accountNum: account_num2, type: 2 },
    ];
    msg = "successfully";
    errorCode = 0;
    ...
    mailController.sentMail(
      data.email,
      "[New Vimo][important !!!] Account Vimo",
      msgText,
      htmlmsg
    );
    const rfToken = rndToken.generate(LENGTH_REFREST_TOKEN);
    refreshTokenModel.add({
...
router.post("/closed", async (req, res) => {
  console.log(req.body);
  let account = await bankingInfoModel.getInfoAccountByAccNum(
    req.body.closerId
  );
  ...
  account = account[0];
  let tagetAcc = await bankingInfoModel.getInfoAccountByAccNum(
    req.body.receiveId
  );
...
router.post("/acc", async (req, res) => {
  console.log("acccccccccccccc", JSON.stringify(req.body));
  let item = {};
  ...
  if (req.body.partner && req.body.partner !== "0" && req.body.partner !== 0) {
    ...
    console.log("sdfsdfsdf getInfoByAccount");
    account = await accountModel.getInfoByAccount(req.body.query);
    console.log("account getInfoByAccount", JSON.stringify(account));
    item = account[0];
  }
  let ret = {
    errorCode: -201,
